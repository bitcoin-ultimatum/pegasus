...
usage()
{
   ...
   echo -e "\t-s Build a static type build (additionally will build from the sources: libnorm, libzmq3)"
...
install_package () {
    ...
    if [ "" = "$PKG_OK" ]; then
     apt-get --assume-yes --force-yes -y install $REQUIRED_PKG
    else 
...
uninstall_package () {
    ...
    if [ "install ok installed" = "$PKG_OK" ]; then
     apt remove --purge --auto-remove --assume-yes --force-yes -y $REQUIRED_PKG
    fi
    ...
    sleep 1
     ...
     rm /var/lib/dpkg/lock-frontend 2> /dev/null
}
...
./bootstrap --prefix=/usr && make -j$(nproc) &&  make install
cd -
...
./bootstrap.sh --prefix=/usr --with-python=python3 &&
 ./b2 stage -j$(nproc) threading=multi link=static,shared --with-regex --with-test --with-filesystem --with-date_time --with-random --with-system --with-thread --with-program_options --with-chrono --with-fiber --with-log --with-context --with-math &&  ./b2 install --prefix=/usr
cd -
...
cd graphite
cmake . -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX:PATH=/usr &&  cmake --build . --target install --config Release
cd -
...
wget wget https://gmplib.org/download/gmp/gmp-6.1.2.tar.xz
tar xvf gmp-6.1.2.tar.xz
cd gmp-6.1.2
./configure && make && make install
[ ! -d "/usr/include/x86_64-linux-gnu/" ] && mkdir /usr/include/x86_64-linux-gnu/
ln -s /usr/local/include/gmp.h /usr/include/x86_64-linux-gnu/gmp.h
cd -
...
cd /usr/src/googletest
 cmake . &&  cmake --build . --target install && cd -
...
if [ "$is_static" = true ]
then
    cmake -G "CodeBlocks - Unix Makefiles" . -DBUILD_STATIC=ON -DENABLE_GUI=OFF -DQUIET=ON
else

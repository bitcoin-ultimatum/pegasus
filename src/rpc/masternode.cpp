    ...
    for (PAIRTYPE(int, CMasternode) & s : vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" && strTxHash.find(strFilter) == std::string::npos &&
                ...
                EncodeDestination(PKHash(mn->pubKeyCollateralAddress.GetID())).find(strFilter) == std::string::npos) continue;
            ...
            obj.push_back(Pair("addr", EncodeDestination(PKHash(mn->pubKeyCollateralAddress.GetID()))));
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:3668\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:3668\""));
...
UniValue masternodecurrent (const UniValue& params, bool fHelp)
{
    ...
    if (winner) {
        ...
        obj.push_back(Pair("pubkey", EncodeDestination(PKHash(winner->pubKeyCollateralAddress.GetID()))));
...
UniValue createmasternodekey (const UniValue& params, bool fHelp)
{
    ...
    auto secretStr = EncodeSecret(secret);
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    ...
    if (pmn) {
        ...
        mnObj.push_back(Pair("addr", EncodeDestination(PKHash(pmn->pubKeyCollateralAddress.GetID()))));
...
UniValue decodemasternodebroadcast(const UniValue& params, bool fHelp)
{
    ...
    resultObj.push_back(Pair("pubkeycollateral", EncodeDestination(PKHash(mnb.pubKeyCollateralAddress.GetID()))));
    resultObj.push_back(Pair("pubkeymasternode", EncodeDestination(PKHash(mnb.pubKeyMasternode.GetID()))));
